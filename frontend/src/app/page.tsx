    ...
    <div className="animate-fade-in">
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-4xl mx-auto text-center px-6">
          ...
          <p className="text-lg md:text-xl text-accent-600 dark:text-accent-300 mb-10 max-w-3xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {user ? (
              <>
                <Link 
                  href="/dashboard" 
                  className="px-8 py-3 bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 transition-colors shadow-button"
                >
                  Go to Dashboard
                </Link>
                <Link 
                  href={`/p/${user.username}`}
                  className="px-8 py-3 bg-secondary-600 text-white rounded-lg font-medium hover:bg-secondary-700 transition-colors shadow-button"
                >
                  View Your Profile
                </Link>
              </>
              ...
              <>
                ...
                <Link 
                  href="/login" 
                  className="px-8 py-3 bg-accent-600 text-white rounded-lg font-medium hover:bg-accent-700 transition-colors shadow-button"
                >
                  Log In
                </Link>
              </>
      ...
      {/* Features Section */}
      <section className="py-16 bg-white dark:bg-accent-800 shadow-soft rounded-xl">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-accent-50 dark:bg-accent-700 p-8 rounded-xl shadow-card hover:shadow-lg transition-shadow">
            ...
            <div className="bg-accent-50 dark:bg-accent-700 p-8 rounded-xl shadow-card hover:shadow-lg transition-shadow">
              ...
              <Link href="/leaderboard" className="text-primary-600 dark:text-primary-400 hover:underline inline-flex items-center mt-3 font-medium text-sm">
                See the leaderboard
      ...
      <section className="py-12 mt-16">
        <div className="max-w-lg mx-auto bg-white dark:bg-accent-800 rounded-xl shadow-soft p-8">
          <h2 className="text-2xl font-heading font-bold text-accent-800 dark:text-white mb-6">API Status</h2>
          <div className={`p-6 rounded-lg ${
            apiStatus === 'connected' ? 'bg-green-50 text-green-800 dark:bg-green-900/20 dark:text-green-300' : 
            apiStatus === 'error' ? 'bg-red-50 text-red-800 dark:bg-red-900/20 dark:text-red-300' : 
